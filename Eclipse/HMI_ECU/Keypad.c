...
uint8 KEYPAD_getPressedKey(void)
{
	...
	while(1)
	{
		...
		{
			/*each time setup the direction for all keypad port as input pin,except for this row will be output pin*/
			...
			{
				/******************** check if the switch pressed in this columns ***********************/
			...
			_delay_ms(5); /* to fix CPU load issue in proteus */
		}
	}
}
...
#if (KEYPAD_NUM_COLS == 3)
/* Description: Update the keypad pressed button value with the correct one in keypad 4x3 shape*/
static uint8 KEYPAD_4x3_adjustKeyNumber(uint8 button_number)
{
...
/* Description: Update the keypad pressed button value with the correct one in keypad 4x4 shape*/
static uint8 KEYPAD_4x4_adjustKeyNumber(uint8 button_number)
{
	uint8 keypad_button = 0;
	switch(button_number)
